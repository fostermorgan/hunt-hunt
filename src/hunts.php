...
$query = 'SELECT * FROM  hunts
                    INNER JOIN animals ON hunts.animal_id = animals.animal_id
                      INNER JOIN users ON hunts.user_id = users.user_id
                        INNER JOIN locations ON hunts.location_id = locations.location_id' . $addUserHuntsToQuery . $addAnimalToQuery . ';';
...
while ($row = @mysql_fetch_assoc($result)){
  ...
  $newnode->setAttribute("id",$row['hunt_id']);
  $newnode->setAttribute("animalName",$row['animalName']);
...
<html>
  <body>
    ...
    <script async defer
    src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAL2wti_wS8G_3VMWmLuV7Ih2MZZu7ZErs&callback=initMap">
    </script>
  </body>
</html>
